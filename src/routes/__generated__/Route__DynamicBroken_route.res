...
@inline
let routePattern = "/dynamic/:item(foo|bar)"

@live
let makeLink = (~item: [#foo | #bar]) => {
  RelayRouter.Bindings.generatePath(routePattern, Js.Dict.fromArray([("item", (item :> string)->Js.Global.encodeURIComponent)]))
}

@live
type pathParam_item = [#foo | #bar]
